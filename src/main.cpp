...
#if defined(NDEBUG)
# error "Cryptium cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcccafbe3527f997991cf557ff27ee7bf515ce7ddd6277735fe7ca55e1a8d4ab7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cryptium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cryptium Signed Message:\n";
...
{
    // Cryptium: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 6000 blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cryptium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cryptium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcccafbe3527f997991cf557ff27ee7bf515ce7ddd6277735fe7ca55e1a8d4ab7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1448626444, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "ny times 112711";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c82f116c3985d14fce9122a4cae934f19d292ea32b21526982dc030433560b79c44601263751c64a185b309cf47bcece016a66fb3dba20dbc7a1b971c952cd63") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1448626444;
        ...
        assert(block.hashMerkleRoot == uint256("0x486709854e4661c7773c5a55845d1561d30022c6411b08be2d5635b77407753f"));
...
unsigned char pchMessageStart[4] = { 0xb1, 0x8a, 0xff, 0xa2 }; // Cryptium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptiumMiner(CWallet *pwallet)
{
    printf("CryptiumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptium-miner");
    ...
    try { loop {
        ...
        printf("Running CryptiumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptiumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptiumMiner, pwallet));
}
